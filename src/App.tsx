...
export default function App() {
  ...
  return (
    <LocaleProvider>
      ...
      /* ── 移动端（<1024px）：单列，隐藏 Heatmap/PersonalBest/CalendarWidget，顺序 ProfileCard→ActivityLog→RouteMap→StatsCards→ChinaMap ── */
      ...
      /* ── 桌面（≥1024px）：两列，全部卡片，保持原布局 ── */
      <main className="max-w-[1400px] mx-auto px-6 py-6">
        <div className={`grid grid-cols-1 lg:grid-cols-[1fr_360px] xl:grid-cols-[1fr_380px] gap-6 ${heatmapDisplayYear === 'all' ? 'items-start' : 'items-stretch'}`}>
          {/* Left column */}
          <div className="flex flex-col gap-6 min-w-0 overflow-hidden">
            ...
            <Reveal y={40} delay={200} className="md:h-[969px]">
              <ActivityLog
                activities={filtered}
                years={years}
                year={year}
      ...
      <footer className="text-center py-6 text-sm text-[var(--color-muted)] border-t border-[var(--color-border)]">
        <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
          ...
          <span>
            Based on{' '}
            <a href="https://github.com/zhaohongxuan/workouts" target="_blank" rel="noopener noreferrer" className="hover:text-[var(--color-text)] transition-colors">
              zhaohongxuan/workouts
            </a>
          </span>
        </div>
      </footer>
